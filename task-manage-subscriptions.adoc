...
* https://docs.netapp.com/us-en/console-setup-admin/task-subscribe-standard-mode.html["NetApp 데이터 서비스(표준 모드) 구독 방법 알아보기"^]
* https://docs.netapp.com/us-en/console-setup-admin/task-subscribe-restricted-mode.html["NetApp 데이터 서비스(제한 모드) 구독 방법 알아보기"^]
...
NOTE: AWS의 경우 계정에 NetApp 마켓플레이스 구독이 여러 개 있을 수 있습니다. 독립형 Cloud Volumes ONTAP 구독만 연결된 경우 Cloud Volumes ONTAP Essentials를 사용하여 Cloud Volumes ONTAP을 배포할 수 있지만 백업 및 복구를 구성하려면 다른 구독이 필요합니다. Cloud Volumes ONTAP 용량 청구에 사용할 구독을 선택하려면(기존 시스템의 청구 변경 포함) *Administration > Licenses and subscriptions > Billing preferences*로 이동하십시오.
...
. NetApp Console 메뉴에서 *관리 > Licenses and subscriptions*을 선택합니다.
. *마켓플레이스 구독*을 선택하세요.
. 연결하려는 구독 행에서 작업 메뉴를 열고 *Associate*를 선택합니다.
. *구독 연결* 대화 상자에서 구독과 연결할 NetApp Console 조직을 선택합니다.
. *연계*를 선택하세요.
...
. NetApp Console 메뉴에서 *관리 > Licenses and subscriptions*을 선택합니다.
. *마켓플레이스 구독*을 선택하세요.
. 자격 증명을 보려는 구독이 있는 행에서 *View*를 선택합니다. 구독에 여러 자격 증명이 연결된 경우 자격 증명이 표시되지 않을 수 있으며, 이 경우 다른 에이전트를 선택해야 합니다.
...
클라우드 공급자(AWS, Google Cloud 또는 Azure)에서 구독을 취소하면 Console에 구독 상태가 *Unsubscribed*로 표시됩니다. 구독을 취소한 후에는 Licenses and subscriptions에서 해당 구독을 제거할 수도 있습니다.

.시작하기 전에
* 구독 상태가 *구독 취소됨*인 구독만 삭제할 수 있습니다. 삭제하기 전에 관련 설정, 데이터 및 구성을 모두 지우거나 재설정해야 합니다.
+
구독이 아직 구성된 경우 *제거* 옵션을 사용할 수 없습니다.
...
. *구독 제거* 대화 상자에서 *제거*를 선택하여 구독을 제거할지 확인합니다.
