...
summary: Google Cloud 콘솔의 NetApp Console 목록에 대한 마켓플레이스 비공개 제안을 수락합니다. 콘솔에서 Google Cloud 자격 증명을 구성합니다. 
---
...
====
.단계
. NetApp 영업팀에서 제공한 비공개 오퍼 활성화 링크를 선택하십시오. 링크는 다음과 유사한 형식입니다.
...
. 정보가 정확한지 확인하고, 약관에 동의한 후 **Accept**를 선택하세요.
. 구독이 생성되는 동안 로딩 아이콘이 나타납니다.
. 구독이 성공적으로 생성되면 팝업 창이 표시됩니다. *주문 요청이 NetApp, Inc.로 전송되었습니다.*
. **NETAPP, Inc.에 등록**을 선택하세요. NetApp Console로 리디렉션되어 구독 구성을 완료할 수 있습니다. 조직 내 다른 사람이 비공개 제안을 수락한 경우:
+
.. Google Cloud Marketplace에서 **주문**으로 이동합니다.
.. 상태가 **보류**인 제안을 찾으세요.
.. 점 세 개 메뉴를 선택하고 **NetApp, Inc.에 등록**을 선택하세요.
...
.. 구독에 대한 액세스 권한을 부여할 Console *Organizations*를 선택하십시오.
.. 선택적으로, 하나의 Console 조직에서 기존 Marketplace 구독을 자동으로 교체하려면 **기존 구독 교체**를 활성화할 수 있습니다.
+
[NOTE]
====
